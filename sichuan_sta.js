...
var x = ee.Geometry.Polygon(
  ...
  // 获取不重复的区域（每个图像的地理位置）
  var distinctImages = sentinelCollection.distinct('system:index');
  
  // 统计不重复区域的图像数量
  var imageCount = distinctImages.size();
  
  // 输出结果
  print('不重复图像数量：', imageCount);
  
  // 裁剪图像到自定义边界
  var sentinelClipped = sentinelCollection.median().clip(x);
